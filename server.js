...
// ============================================================
try { require('dotenv').config(); } catch(_) {}  // loads .env locally; no-op on Railway
...
app.post('/api/createOrder', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    if (!order.id) {
      ...
      return res.status(500).json({ error: 'Razorpay order creation failed: ' + rzpMsg });
    }
...
// Subscription plan amounts (months → expected paise)
const PLAN_AMOUNTS = { 1:10000, 2:20000, 3:30000, 4:40000, 6:50000, 12:100000 };

// POST /api/verifyPayment — verifies signature + confirms payment with Razorpay API
app.post('/api/verifyPayment', authMiddleware, async (req, res) => {
  ...
  // This signature can ONLY be generated by Razorpay after a successful payment capture
  const body = razorpay_order_id + '|' + razorpay_payment_id;
  const expectedSig = crypto.createHmac('sha256', RAZORPAY_KEY_SECRET).update(body).digest('hex');
  if (expectedSig !== razorpay_signature) {
    console.warn(`[Razorpay] Signature mismatch for payment ${razorpay_payment_id} — user ${req.user.id}`);
    return res.status(400).json({ error: 'Payment verification failed: invalid signature' });
  }

  // ── Step 2: Confirm payment status directly with Razorpay API ─
  // Prevents any edge-case where a payment_id from a failed/refunded transaction is replayed
  try {
    const payment = await razorpayRequest('GET', `/payments/${razorpay_payment_id}`);
    console.log(`[Razorpay] Payment ${razorpay_payment_id} status: ${payment.status}, amount: ${payment.amount}`);

    if (payment.status !== 'captured' && payment.status !== 'authorized') {
      console.warn(`[Razorpay] Payment not captured — status: ${payment.status}`);
      return res.status(400).json({ error: `Payment not successful (status: ${payment.status}). Subscription not activated.` });
    }

    // ── Step 3: Verify amount matches the plan ─────────────────
    const expectedPaise = PLAN_AMOUNTS[parseInt(months)];
    if (expectedPaise && payment.amount !== expectedPaise) {
      console.warn('[Razorpay] Amount mismatch — expected ' + expectedPaise + ', got ' + payment.amount);
      return res.status(400).json({ error: 'Payment amount mismatch. Please contact support.' });
    }

    // Step 4: Activate subscription
    await pool.query(
      `UPDATE users SET subscription_paid_till = GREATEST(COALESCE(subscription_paid_till, NOW()), NOW()) + ($1 || ' months')::interval, status = 'active' WHERE id = $2`,
      [months, req.user.id]
    );
    const user = await pool.query(
      `SELECT id, name, email, status, trial_expires_on, subscription_paid_till FROM users WHERE id = $1`,
      [req.user.id]
    );
    console.log('[Razorpay] Subscription activated for user ' + req.user.id + ' — ' + months + ' month(s)');
    res.json({ success: true, user: user.rows[0] });

  } catch(e) {
    console.error('[Razorpay] verifyPayment error:', e.message);
...
(async () => {
  try {
    ...
    await pool.query('CREATE INDEX IF NOT EXISTS idx_progress_year ON progress(year)');
    await pool.query('CREATE INDEX IF NOT EXISTS idx_progress_user_year ON progress(user_id, year)');
    console.log('DB indexes OK');
  } catch(e) { console.log('Index check:', e.message); }
})();

app.listen(PORT, () => console.log('UPPSC Study Portal started on port ' + PORT));
